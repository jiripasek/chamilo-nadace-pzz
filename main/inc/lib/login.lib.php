...
class Login 
{
	...
	public static function handle_encrypted_password($user, $by_username = false) {	
		...
		$email_body = get_lang('DearUser')."<br>\n".get_lang('password_request')."\n";
		$email_body .= $user_account_list."\n-----------------------------------------------\n\n";
		//$email_body .= get_lang('PasswordEncryptedForSecurity');
		$email_body .= "<br><br>S pozdravem<br>administrátor e-learningu edupolis";
	...
	public static function reset_password($secret, $id, $by_username = false) {
		...
		if (self::get_secret_word($user['email']) == $secret) { // OK, secret word is good. Now change password and mail it.
			...
			$result = Database::query($sql);
			//SG
		$sql = "SELECT `email` FROM ".$tbl_user." WHERE `user_id`='$id' LIMIT 1";
		
		$result = Database::query($sql);
		$array_result = mysql_fetch_array($result);
		$emailxxx = $array_result[0];
		$sql = "UPDATE `registrovani` SET `heslo`='$crypted' WHERE `email`='$emailxxx' LIMIT 1";
		Database::query($sql);
