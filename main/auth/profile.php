...
//	$user_data = $form->exportValues();
   ...
   elseif (isset($password) && isset($changeemail) && in_array('email', $available_values_to_modify) && in_array('password', $available_values_to_modify)) {
		...
		//SG
		$emailx_array = mysql_fetch_array(mysql_query("SELECT `email` FROM `user` WHERE `user_id`= '".api_get_user_id()."' LIMIT 1;"));
		$emailx = $emailx_array[0]; 
		mysql_query("UPDATE `registrovani` SET `heslo` = '".Database::escape_string($password)."' WHERE `email` = '$emailx' AND  `stornovana` = '0'  LIMIT 1");
