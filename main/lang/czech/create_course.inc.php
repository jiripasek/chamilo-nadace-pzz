<?php
/*
CZ - OK
for more information: see languages.txt in the lang folder.
*/
$langProfessors = "Učitelé";
$langExplanation = "Jedním kliknutím na OK vytvoříte webstránku s Fórem, Agendou, Správou dokumentů atd. Vaše přihlášení, jako tvorce webstránky Vám umožní změnit ji dle Vaší potřeby.";
$langEmpty = "Nechali jste některá políčka prázdna.<br>Použijte tlačítko <b>Zpět</b> na Vašem prohlížeči a skuste to znovu.<br>Jestli ignorujete kód kurzu, podívejte se na Program kurzu";
$langCodeTaken = "Tento kód kurzu byl už použit. <br>Použijte tlačítko <b>Zpět</b> na Vašem prohlížeči a skuste to znovu";
$langFormula = "S přátelským pozdravem";
$langMessage = "Když odstraníte test fórum, odstraníte také všechny odkazy které se v tomto fóru nachází.";
$langExerciceEx = "Vzorový test";
$langAntique = "Ironie";
$langSocraticIrony = "Sokratická ironie je...";
$langManyAnswers = "(může být správna více než jedna odpověď)";
$langRidiculise = "Zesměšňuje účastníka rozmluvy aby ho donútil připust že nemá pravdu.";
$langNoPsychology = "Ne. Sokratická ironie nezávisý na psychologii, týká se argumentace.";
$langAdmitError = "Připustit si jeden vlastný omyl aby doshál toho, že řečník udělá to samé.";
$langNoSeduction = "Ne. Sokratická ironie není strategie k svádení nebo metoda založena na příkladech.";
$langForce = "Přinutit jednoho účastníka rozpravy pomocí série otázek a podotázek, aby si přiznal že neví nic o tom co tvrdil že ví.";
$langIndeed = "Skutečně. Sokratická ironie je dotazovací metoda. Grécké \"eirotao\" znamená \"klást otázky\"";
$langContradiction = "Použitím Principu bez protiřečení zaženete účastníka rozmluvy do slepé uličky.";
$langNotFalse = "Odpověd není špatná. Je pravda že odhalení nevědomosti účastníka rozmluvy znamená poukázání na protichůdné závěry kam vedl své předpoklady.";
$langAgenda = "Agenda";
$langVideo = "Video";
$langCourseProgram = "Popis kurzu";
$langAnnouncements = "Oznámení";
$langAddPageHome = "Nahrát stránku a spoj(link) s Domácí stránkou";
$langLinkSite = "Přidat odkaz stránky na Domácí stránku";
$langModifyInfo = "Nastavení kurzu";
$langCourseDesc = "Popis kurzu";
$langAgendaTitle = "Uterý 11. Prosince  - První setkání. Místnost: LIN 18";
$langAgendaText = "Všeobecní úvod do projektového managementu";
$langMicro = "Pouliční rozhovory";
$langGoogle = "Rychlý a účinný vyhledávač";
$langIntroductionText = "Toto je úvodní text. Pro nahrazení Vašim vlastním textem, klikněte dolů na ikonu s tužkou (pouze pro administrátory kurzu)";
$langIntroductionTwo = "Tato stránka dovoluje uživatelům a skupinám publikování dokumentů.";
$langCourseDescription = "Popis kurzu";
$langProfessor = "Učitel";
$langAnnouncementEx = "Toto je jenom příklad oznámení. Jenom učitelům kurzu je dovoleno publikovat oznámení.";
$langJustCreated = "Práve jste vytvořil oblast kurzu";
$langEnter = "Zpět na můj seznam kurzů";
$langGroups = "Skupiny";
$langCreateCourseGroups = "Skupiny";
$langCatagoryMain = "Hlavní";
$langCatagoryGroup = "Skupinové fóra";
$langLn = "Jazyk";
$langCreateSite = "Vytvořit oblast kurzu";
$langFieldsRequ = "Všechny políčka jsou požadována";
$langEx = "např. <i>Inovační management</i>";
$langFac = "Kategorie";
$langTargetFac = "Toto je oddělení nebo jiná kategorie, kde je kurz zařazený";
$langMax = "max. 20 znaků, např. <i>INNOV21</i>";
$langDoubt = "Jestli máte pochybnosti o kódu kurzu, poraďte se s administrátorem";
$langProgram = "Program kurzu</a>. Jestli Váš kurz nemá kód a to z jakéhokoliv důvodu, vytvořte mu ho. Například <i>INOVACE</i> když se kurz týká Inovačního Managementu";
$langScormtool = "Učební osnova";
$langScormbuildertool = "Vytváření SCORM učební osnovy";
$langPathbuildertool = "Tvorba učební osnovy";
$langOnlineConference = "Konference";
$langAgendaCreationTitle = "Vytvoření kurzu";
$langAgendaCreationContenu = "Tento kurz byl vytvořen v tento moment.";
$langOnlineDescription = "Popis konference (On-line)";
$langDropbox = "Schránka dokumentů";
$langOnly = "Pouze";
$langRandomLanguage = "Náhodný výběr v dostupných jazycích";
$langImages = "Obrázky";
$langAudio = "Audio";
$langFlash = "Flash";
$langForumLanguage = "Anglicky";
$langNewCourse = "Nová Oblast kurzu";
$langAddNewCourse = "Přidat novou oblast kurzu";
$langRestoreCourse = "Obnovit kurz";
$langOtherProperties = "Ostatní vlastnosti nalezené v archivu";
$langSysId = "ID Systému";
$langDepartment = "Oddělení";
$langDepartmentUrl = "URL oddělení";
$langScoreShow = "Ukaž výsledek";
$langVisibility = "Viditelnost";
$langVersionDb = "Verze databáze je použitá v čase archivu";
$langLastVisit = "Posledně navštívené";
$langLastEdit = "Posledně editované";
$langExpire = "Expirace";
$langChoseFile = "Vyber soubor";
$langFtpFileTips = "Soubor na FTP serveru";
$langHttpFileTips = "Soubor na Web (HTTP) serveru";
$langLocalFileTips = "Soubor na platforme serveru";
$langPostFileTips = "Soubor na Vašem lokálním počítači";
$langOtherCategory = "Jiná kategorie";
$langMinimum = "minimum";
$langMaximum = "maximum";
$langRestoreACourse = "obnovit kurz";
$langBackup = "Záloha";
$langCopy = "Kopírovat obsah kurzu";
$langRecycle = "Recyklovat kurz";
$AnnouncementExampleTitle = "Toto je přiklad oznámení";
$Wikipedia = "Online encyklopedie - bezplatná";
$DefaultGroupCategory = "Předvolené skupiny";
$DefaultCourseImages = "Galerie";
$ExampleForumCategory = "Příklad kategorie fóra";
$ExampleForum = "Příklad fóra";
$ExampleThread = "Příklad vlákna";
$ExampleThreadContent = "Příklad obsahu";
$IntroductionWiki = "Slovo Wiki je zkratka pro WikiWikiWeb. Wikiwiki je havajské slovo, znamenající rychlý nebo rychlost. Ve wiki lidé píšou stránky společně. Pokud někdo něco napíše chybně, pak jej může další osoba opravit. Další osoba může přidat další informaci. Tím jsou stránky neustále vylepšovány.";
$CreateCourseArea = "Vytvořit tento kurz";
$CreateCourse = "Vytvořit kurz";
$Create = "Vytvořit";
$MessageOfNewCourseToAdmin = "Tento vzkaz je pro informaci o vytvoření nového kurzu na platformě";//SGV
$NewCourseCreatedIn = "Nový kurz vytvořen";//SGV
$ExplicationTrainers = "Cvičitel je nastaven nyní pro Vás. Později můžete změnit nastavení v tréninkové konfiguraci nastavení";//SGV
$Objectives = "Záměry";//SGV
$TargetAudience = "Cílový posluchači";//SGV
$YouHaveToAcceptTermsAndConditions = "Musíte přijmout naše termíny a podmínky nebo požádat o nový trénink.";//SGV
$CourseRequestCreated = "Vaše žádost o nový kurz byla úspěšně odeslána. Brzy obdržíte odpověď, během jednoho nebo dvou dnů.";//SGV
$CreateThisCourseRequest = "Vytvořit požadavek na tento kurz";//SGV
$CourseRequestDate = "požadavek termínu";//SGV
$AcceptThisCourseRequest = "Přijmout tento kurz";//SGV
$ANewCourseWillBeCreated = "Nový kurz bude vytvořen. Pokračovat?";//SGV
$AdditionalInfoWillBeAsked = "Additional information about %s training request is going to be asked through an e-mail message. Is it OK to proceed?";//TODO
$AskAdditionalInfo = "Požádat pro přídavné informace";//SGV
$DeleteThisCourseRequest = "Smazat tento pžadavek na kurz";//SGV
$ACourseRequestWillBeDeleted = "Požadavek na kurz bude smazán. Pokračovat?";//SGV
$RejectThisCourseRequest = "Odmítnout tento požadavek na kurz";//SGV
$ACourseRequestWillBeRejected = "Požadavek na kurz bude osmítnuta. Pokračovat?";//SGV
$CourseRequestAccepted = "Žádost o kurz byla přijatá. Nový kurz byl vytvořen.";//SGV
$CourseRequestAcceptanceFailed = "Žádost na kurz nebyla přijata kvůli vnitřní chybě.";//SGV
$CourseRequestRejected = "Žádost o kurz byla odmítnuta.";//SGV
$CourseRequestRejectionFailed = "Žádost o kurz nebyla odmítnuta díky vnitřní chybě.";//SGV
$CourseRequestInfoAsked = "Přídavná informace ke kurzu byla poslána.";//SGV
$CourseRequestInfoFailed = "Přídavná informace ke kurzu nebyla poslána díky vnitřní chybě.";//SGV
$CourseRequestDeleted = "Žádost o kutz byla smazánna.";//SGV
$CourseRequestDeletionFailed = "Žádost o kurz nebyla smazána díky vnitřní chybě.";//SGV
$DeleteCourseRequests = "Smazat vybraný kurz";//SGV
$SelectedCourseRequestsDeleted = "Požadavky pro vybraný kurz byly smazány.";//SGV
$SomeCourseRequestsNotDeleted = "Některé požadavky k vybranému kurzu nebyly smazány díky vnitřní chybě.";//SGV
$CourseRequestEmailSubject = "Žádost o nový kurz";//SGV
$CourseRequestMailOpening = "Zaregistrovali jsme následující požadavky pro nový kurz:";//SGV
$CourseRequestPageForApproval = "Tento požadavek pro kurz bude schválen na další stránce:";//SGV
$PleaseActivateCourseValidationFeature = "Ověření správnosti kurzu není zaktivováno okamžitě. V případě užití této funkce, prosím, aktivujte použitím nastavení.";//SGV
$CourseRequestLegalNote = "Informace o požadavcích na kurz jsou považovány za chráněné; mohou být použity pouze v případě otevření nového kurzu s naším e-learning portálem; neměly by být předány třetí straně.";//SGV
$CourseRequestAskInfoEmailSubject = "Žádost o přidání informace o požadavcích kurzu";//SGV
$CourseRequestAskInfoEmailText = "Obdrželi jsme Váš požadavek pro nový kurz s kódem. Předtím než zvážíme schválení, potřebujeme některé přídatné informace.Prosím, poskytněte stručné informace o náplni kurzu (popis), záměry, studenty nebo uživatele jež mají být zahrnuty do návrhu kurzu. Je-li to aplikovatelné, uveďte jména institucí nebo jednotek pro jejichž podporu jste dal požadavek kurzu.";//SGV
$CourseRequestAcceptedEmailSubject = "Požadavek na kurz byl schválen";//SGV
$CourseRequestAcceptedEmailText = "Vaše žádost na kurz byla schválena. Nový kurz byl vytvořen a vy jste registrovaný jako učitel.Nyní můžete zpřístupnit nový kurz";//SGV
$CourseRequestRejectedEmailSubject = "Žádost o kurz byla odmítnuta";//SGV
$CourseRequestRejectedEmailText = "K naší lítosti Vám oznamujeme že Vaše žádost o kurz byla zamítnuta pro nevyplnění požadavků našich termínů a podmínek.";//SGV
$CourseCreationFailed = "Kurz nebyl vytvořen pro vnitřní chybu.";//SGV
$CourseRequestCreationFailed = "Požadavek na kurz nebyl vytvořen pro vnitřní chybu.";//SGV
$CourseRequestEdit = "Sestavit žádost na kurz";//SGV
$CourseRequestHasNotBeenFound = "Žádost na kurz ke zpřístupnění nebyl nalezen nebo neexistuje.";//SGV
$Accept = "Přijmout";//SGV
$Reject = "Odmítnout";//SGV
$CourseRequestUpdateFailed = "Požadavek na kurz nebyl aktualizován pro vnitřní chybu.";//SGV
$CourseRequestUpdated = "Požadavek na kurz byl aktualizován.";//SGV
?>
